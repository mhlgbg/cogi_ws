...
import {
  ...
  normalizeApiRows,
  toPositiveNumber,
} from "../service-orders/serviceOrderFormatters"
...
import { getSalesCounterContext } from "../../api/salesCounterApi"
...
export default function SalesCounterPage() {
  ...
  const roleText = normalizeRoleName(role)
  const isSalesCounterRole = roleText === "sales counter" || roleText === "sales-counter" || roleText === "salescounter"
  const isAdminRole = roleText === "admin"

  const canCreateOrder = can("serviceSales.order.create") || can("service-orders") || isSalesCounterRole || isAdminRole
  const canCreateCustomer = can("customers") || can("service-orders") || isSalesCounterRole || isAdminRole
  const canViewRecentOrders =
    can("service-orders") ||
    canAny([
                                      ...
                                      <tr key={row.id} className={row.id === highlightOrderId ? "table-warning" : ""}>
                                        ...
                                        <td>
                                          <CButton
                                            ...
                                            color="info"
                                            variant="outline"
                                            onClick={() => navigate(`/service-orders/${row.id}`)}
                                          >
                                            Chi tiết
                                          </CButton>
                                        </td>
