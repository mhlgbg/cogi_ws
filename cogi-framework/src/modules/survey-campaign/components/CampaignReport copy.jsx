...
import {
  exportSurveyCampaignAnswersReport,
...
export default function CampaignReport({ campaignId, active, reloadKey = 0 }) {
  ...
  async function handleExportAllAnswers() {
    if (!campaignId || exportingAnswers) return

    setExportingAnswers(true)
    setError('')

    try {
      const result = await exportSurveyCampaignAnswersReport(campaignId)
      downloadBlob(result?.blob, result?.fileName || `survey-answers-${campaignId}.xlsx`)
    } catch (requestError) {
      setError(getApiMessage(requestError, 'Không thể export toàn bộ câu trả lời'))
    } finally {
            ...
            <CCardBody>
              <div className='d-flex justify-content-between align-items-center gap-2 mb-3'>
                ...
                <div className='d-flex gap-2 flex-wrap justify-content-end'>
                  ...
                  <CButton size='sm' color='primary' variant='outline' onClick={handleExportAllAnswers} disabled={exportingAnswers || !campaignId}>
                    {exportingAnswers ? 'Đang xuất...' : 'Xuất tất cả câu trả lời'}
                  </CButton>
                </div>
              </div>
              {lecturers.length === 0 ? (
