...
import { CAlert, CBadge, CButton, CCard, CCardBody, CPagination, CPaginationItem, CSpinner } from '@coreui/react'
import { listMembershipHistory } from '../services/clubMembershipHistoryService'
import {
...
function getEventLabel(eventType) {
  ...
  if (key === 'info_updated') return 'Cập nhật thông tin'
  return 'Sự kiện khác'
}

function HistoryCard({ item }) {
  const statusFrom = getClubMembershipStatusMeta(item.fromStatus)
  const statusTo = getClubMembershipStatusMeta(item.toStatus)
  ...
  return (
    <CCard className='mb-3'>
      <CCardBody>
        ...
        {item.fromStatus || item.toStatus ? <div className='mb-2'>Trạng thái: <CBadge color={statusFrom.color}>{statusFrom.label}</CBadge> {' -> '} <CBadge color={statusTo.color}>{statusTo.label}</CBadge></div> : null}
...
export default function ClubMembershipHistorySection({ membershipId }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError('')
      try {
        const result = await listMembershipHistory(membershipId, { page: pagination.page, pageSize: pagination.pageSize, sort: 'eventAt:desc' })
        if (!mounted) return
        ...
        if (!mounted) return
        setRows([])
        setError(requestError?.response?.data?.error?.message || requestError?.message || 'Không tải được lịch sử membership.')
      } finally {
  ...
  }, [membershipId, pagination.page, pagination.pageSize])
  ...
  return (
    <div className='mt-4'>
      ...
      {loading ? (
        ...
        <>
          {rows.map((item) => <HistoryCard key={item.id} item={item} />)}
            ...
            <div className='d-flex justify-content-end'>
              <CPagination>
                ...
                {pages.map((entry, index) => entry === '...'
                  ? <CPaginationItem key={`membership-history-ellipsis-${index}`} disabled>...</CPaginationItem>
                  : <CPaginationItem key={`membership-history-page-${entry}`} active={pagination.page === entry} onClick={() => setPagination((current) => ({ ...current, page: entry }))}>{entry}</CPaginationItem>)}
