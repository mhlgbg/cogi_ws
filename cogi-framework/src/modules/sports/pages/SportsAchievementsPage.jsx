...
import { listSportsAchievements } from '../services/sportsAchievementService'
import {
...
export default function SportsAchievementsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError('')
      try {
        const result = await listSportsAchievements({ page: pagination.page, pageSize: pagination.pageSize, sort: 'verifiedAt:desc', ...appliedFilters })
        if (!mounted) return
        ...
        if (!mounted) return
        setRows([])
        setError(requestError?.response?.data?.error?.message || requestError?.message || 'Không tải được danh sách achievement.')
      } finally {
  ...
  const basePath = tenantCode ? `/t/${encodeURIComponent(tenantCode)}/sports/achievements` : '/sports/achievements'

  return (
    <div>
      <div className='d-flex justify-content-between align-items-start gap-3 flex-wrap mb-4'>
        <div>
          <div className='fs-5 fw-semibold'>Sports Achievements</div>
          <div className='text-body-secondary'>Quản lý các thành tích đã được ghi nhận hoặc đã thu hồi trong tenant hiện tại.</div>
        </div>
        <CButton color='primary' onClick={() => navigate(`${basePath}/new`)}>Tạo achievement</CButton>
      </div>
      ...
      <CRow className='g-3 mb-3'>
        ...
        <CCol lg={2} md={6}><CFormSelect value={filters.status} onChange={(event) => setFilters((current) => ({ ...current, status: event.target.value }))}><option value=''>Tất cả trạng thái</option>{ACHIEVEMENT_STATUS_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</CFormSelect></CCol>
        <CCol lg={2} md={6}><CFormSelect value={filters.source} onChange={(event) => setFilters((current) => ({ ...current, source: event.target.value }))}><option value=''>Tất cả nguồn</option>{ACHIEVEMENT_SOURCE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</CFormSelect></CCol>
      ...
      <div className='small text-body-secondary mb-3'>Tổng cộng {pagination.total} achievement</div>
      {error ? <CAlert color='danger'>{error}</CAlert> : null}

      {loading ? (
        <div className='d-flex align-items-center gap-2'><CSpinner size='sm' />Đang tải achievement...</div>
      ) : rows.length === 0 ? (
        <CAlert color='secondary' className='mb-0'>Chưa có achievement phù hợp.</CAlert>
      ) : (
        <>
          <CTable responsive hover align='middle'>
            ...
            <CTableBody>
              {rows.map((row) => {
                const statusMeta = getAchievementStatusMeta(row.status)
                return (
                  <CTableRow key={row.id}>
                    ...
                    <CTableDataCell>{getAchievementSourceLabel(row.source)}</CTableDataCell>
                    <CTableDataCell><CBadge color={statusMeta.color}>{statusMeta.label}</CBadge></CTableDataCell>
                    <CTableDataCell>{formatSportsDateTime(row.verifiedAt)}</CTableDataCell>
            ...
            <div className='d-flex justify-content-end'>
              <CPagination>
                ...
                {pages.map((entry, index) => entry === '...'
                  ? <CPaginationItem key={`sports-achievement-ellipsis-${index}`} disabled>...</CPaginationItem>
                  : <CPaginationItem key={`sports-achievement-page-${entry}`} active={pagination.page === entry} onClick={() => setPagination((current) => ({ ...current, page: entry }))}>{entry}</CPaginationItem>)}
