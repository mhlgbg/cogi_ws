...
function extractAvatarUrl(author) {
  const direct = String(author?.avatar?.url || '').trim()
  if (direct) return direct
  const attrs = String(author?.avatar?.attributes?.url || '').trim()
  if (attrs) return attrs
  const dataUrl = String(author?.avatar?.data?.url || '').trim()
  if (dataUrl) return dataUrl
  const dataAttrs = String(author?.avatar?.data?.attributes?.url || '').trim()
...
export default function AuthorManagementPage() {
  ...
  async function load() {
    ...
    try {
      const res = await getAuthors({ page, pageSize, q })
      setRows(res?.data || [])
      setMeta(res?.meta || null)
    } catch (loadError) {
      setRows([])
      setMeta(null)
      setError(getApiMessage(loadError, 'Không tải được danh sách author'))
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (editingId) {
        ...
        setSuccess('Cập nhật author thành công')
      } else {
        await createAuthor(payload)
        setSuccess('Thêm mới author thành công')
      }

      closeModal()
      await load()
    } catch (submitError) {
      setError(getApiMessage(submitError, 'Không thể lưu author'))
    } finally {
  ...
  async function handleDelete(id) {
    if (!window.confirm('Bạn chắc chắn muốn xóa author này?')) return
    ...
    try {
      ...
      setSuccess('Xóa author thành công')
      await load()
    } catch (deleteError) {
      setError(getApiMessage(deleteError, 'Không thể xóa author'))
    }
  }
    ...
    <CRow className='g-0'>
      <CCol xs={12}>
                ...
                <CTable hover responsive className='mb-3 ai-table'>
                  <CTableHead>
                    <CTableRow>
                      ...
                      <CTableHeaderCell style={{ width: 90 }}>Avatar</CTableHeaderCell>
                      <CTableHeaderCell>Tên author</CTableHeaderCell>
                      <CTableHeaderCell style={{ width: 260 }}>Email</CTableHeaderCell>
        ...
        <CModal backdrop='static' visible={showModal} onClose={closeModal} size='lg'>
          <CModalHeader>
            <CModalTitle>{editingId ? 'Sửa author' : 'Thêm mới author'}</CModalTitle>
          </CModalHeader>
          <CModalBody>
            <CForm>
              <CRow className='g-3'>
                <CCol md={6}>
                  <CFormLabel htmlFor='author-name'>Tên <span className='text-danger'>*</span></CFormLabel>
                  <CFormInput
                    id='author-name'
                    value={formData.name}
                ...
                <CCol md={6}>
                  <CFormLabel htmlFor='author-email'>Email</CFormLabel>
                  <CFormInput
                    id='author-email'
                    type='email'
