            ...
            <CRow className='g-3'>
              ...
              <CCol xs={12}>
                ...
                {normalizeStimulusContentType(questionForm.questionTextType) === 'html'
                  ? <SimpleHtmlEditor label='' value={questionForm.questionText} onChange={(value) => setQuestionForm((prev) => ({ ...prev, questionText: value }))} disabled={saving} rows={8} showImageControls={false} allowHtmlMode helperText='HTML đơn giản sẽ được sanitize an toàn khi lưu.' />
                  : <CFormTextarea rows={4} value={questionForm.questionText} onChange={(event) => setQuestionForm((prev) => ({ ...prev, questionText: event.target.value }))} disabled={saving} />}
              </CCol>
