...
export default function DepartmentPage() {
  ...
  const canReadEmployeesForManager = Boolean(
    feature?.hasFeature?.('employee.list')
    || feature?.hasFeature?.('employee.manage')
    || feature?.hasFeature?.('employee.home'),
  )
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (toText(filters.scopeType)) {
        params['filters[scopeType][$eq]'] = filters.scopeType
      }

      const result = await getDepartmentPage(params)
      ...
      setError(apiMessage || 'Không tải được danh sách phòng ban')
      setRows([])
      setTotal(0)
      setPageCount(1)
    } finally {
  ...
  }, [filters.keyword, filters.scopeType, filters.status, page, pageSize])
  ...
  async function handleSave(payload) {
    ...
    try {
      ...
      if (identifier) {
        await updateDepartment(identifier, payload)
      ...
      const apiMessage = requestError?.response?.data?.error?.message
      setError(apiMessage || 'Không thể lưu dữ liệu phòng ban')
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleDelete(row) {
    ...
    const confirmed = window.confirm('Bạn có chắc muốn xóa phòng ban này?')
    if (!confirmed) return
    ...
    try {
      await deleteDepartment(identifier)
      setSuccess('Xóa phòng ban thành công.')
      await Promise.all([loadData(), loadOptions()])
    } catch (requestError) {
      const apiMessage = requestError?.response?.data?.error?.message
      setError(apiMessage || 'Không thể xóa phòng ban')
    }
  }
  ...
  return (
    <CRow className="g-3">
              ...
              <CCol md={3}>
                ...
                <CFormSelect
                  ...
                  onChange={(event) => {
                    const value = event.target.value
                    setPage(1)
                    setFilters((prev) => ({ ...prev, scopeType: value }))
                  }}
                >
                ...
                <CTable hover responsive align="middle">
                  <CTableHead>
                    <CTableRow>
                      ...
                      <CTableHeaderCell scope="col">Scope Type</CTableHeaderCell>
                      <CTableHeaderCell scope="col">Parent Department</CTableHeaderCell>
                    ...
                    {rows.length === 0 ? (
                      ...
                      return (
                        <CTableRow key={toEntityKey(row) || `${row.code}-${index}`}>
                          ...
                          <CTableDataCell>{row.scopeType || '-'}</CTableDataCell>
                          <CTableDataCell>{row.parent?.name || row.parent?.code || '-'}</CTableDataCell>
                          <CTableDataCell>{row.manager?.name || row.manager?.fullName || row.manager?.code || '-'}</CTableDataCell>
                          <CTableDataCell>
      ...
      <DepartmentFormModal
        ...
        allowManagerSelection={canReadEmployeesForManager}
        tenantLabel={tenantName || tenantCode}
      />
    </CRow>
  )
}
