...
import {
  ...
  downloadCandidateExamScoreImportTemplate,
...
export default function CandidateExamScoreImportModal({
  visible,
  admissionSeason,
  onClose,
  onImported,
  ...
  async function handleDownloadTemplate() {
    ...
    try {
      const result = await downloadCandidateExamScoreImportTemplate()
      ...
      setError(getApiMessage(requestError, 'Không thể tải file mẫu import điểm'))
    } finally {
      setDownloadingTemplate(false)
    }
  }
  ...
  async function handlePreview() {
    ...
    try {
      const result = await previewCandidateExamScoreImport({
      ...
      setPreviewResult(null)
      setError(getApiMessage(requestError, 'Không thể xem trước dữ liệu import điểm'))
    } finally {
      setPreviewLoading(false)
    }
  }
  ...
  async function handleConfirm() {
    ...
    try {
      const result = await confirmCandidateExamScoreImport({
        admissionSeasonId: admissionSeason.id,
        file,
      })
      onImported?.(result)
    } catch (requestError) {
      setError(getApiMessage(requestError, 'Không thể xác nhận import điểm'))
    } finally {
      setConfirmLoading(false)
    }
  }
  ...
  return (
    <CModal visible={visible} backdrop='static' size='xl' scrollable onClose={() => !previewLoading && !confirmLoading && !downloadingTemplate && onClose?.()}>
      <CModalHeader>
        <CModalTitle>Import điểm thí sinh</CModalTitle>
      </CModalHeader>
      <CModalBody>
        <div className='small text-body-secondary mb-2'>Kỳ tuyển sinh đang làm việc: {admissionSeason?.name || '-'}</div>
        <div className='small text-body-secondary mb-3'>File mẫu gồm các cột: <strong>Số báo danh</strong>, <strong>Điểm Toán</strong>, <strong>Điểm Tiếng Việt</strong>, <strong>Điểm Tiếng Anh</strong>, <strong>Điểm Khuyến Khích</strong>, <strong>Tổng điểm</strong>.</div>
        ...
        <div className='d-flex flex-wrap gap-2 mb-3'>
          <CButton color='secondary' variant='outline' onClick={handleDownloadTemplate} disabled={downloadingTemplate || previewLoading || confirmLoading}>
            {downloadingTemplate ? 'Đang tải mẫu...' : 'Tải file mẫu điểm'}
          </CButton>
        </div>
            ...
            <CTable hover responsive align='middle'>
              <CTableHead>
                <CTableRow>
                  ...
                  <CTableHeaderCell>Điểm Tiếng Việt</CTableHeaderCell>
                  <CTableHeaderCell>Điểm Tiếng Anh</CTableHeaderCell>
              ...
              <CTableBody>
                {(previewResult.rows || []).slice(0, 100).map((row) => (
                  <CTableRow key={`candidate-exam-score-import-${row.rowIndex}-${row.normalizedData?.candidateNumber || ''}`}>
                    <CTableDataCell>{row.rowIndex || '-'}</CTableDataCell>
                    <CTableDataCell>{row.normalizedData?.candidateNumber || '-'}</CTableDataCell>
                    <CTableDataCell>{row.normalizedData?.mathScore ?? '-'}</CTableDataCell>
                    <CTableDataCell>{row.normalizedData?.vietnameseScore ?? '-'}</CTableDataCell>
                    <CTableDataCell>{row.normalizedData?.englishScore ?? '-'}</CTableDataCell>
