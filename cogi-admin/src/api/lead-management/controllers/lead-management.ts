...
function handleError(ctx: any, error: any) {
  ...
  const message = typeof error?.message === 'string' && error.message.trim()
    ? error.message.trim()
    : 'Unexpected lead management error';
  ...
  strapi.log.error('[lead-management] unexpected error', error);
...
export default {
  async listCampaigns(ctx: any) {
    ...
    try {
      const data = await listLeadCampaigns(ctx.request?.query || {}, getTenantIdFromContext(ctx));
  ...
  async campaignDetail(ctx: any) {
    ...
    try {
      const data = await getLeadCampaignDetail(ctx.params?.id, getTenantIdFromContext(ctx));
      ctx.body = { success: true, data };
    } catch (error: any) {
      return handleError(ctx, error);
    }
  ...
  async campaignFormOptions(ctx: any) {
    ...
    try {
      const data = await getLeadCampaignFormOptions(getTenantIdFromContext(ctx));
      ctx.body = { success: true, data };
    } catch (error: any) {
      return handleError(ctx, error);
    }
  ...
  async createCampaign(ctx: any) {
    ...
    try {
      const data = await createLeadCampaign(ctx.request?.body || {}, getTenantIdFromContext(ctx));
      ctx.body = { success: true, data };
    } catch (error: any) {
      return handleError(ctx, error);
    }
  ...
  async updateCampaign(ctx: any) {
    ...
    try {
      const data = await updateLeadCampaign(ctx.params?.id, ctx.request?.body || {}, getTenantIdFromContext(ctx));
      ctx.body = { success: true, data };
    } catch (error: any) {
      return handleError(ctx, error);
    }
