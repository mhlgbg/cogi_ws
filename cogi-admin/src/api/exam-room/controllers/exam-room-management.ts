...
import { createExamRoom, getExamRoomDetail, handleExamScheduleManagementError, listExamRooms, setExamRoomActive, updateExamRoom } from '../../exam-schedule/services/exam-schedule-management';
...
export default {
  async list(ctx: any) {
    ...
    try { ctx.body = await listExamRooms(Number(resolveCurrentTenantId(ctx)), ctx.query || {}); } catch (error) { return handleExamScheduleManagementError(ctx, error); }
  },
  async detail(ctx: any) {
    const authUser = await requireAuthenticatedUser(ctx); if (!authUser?.id) return;
    try { ctx.body = { success: true, data: await getExamRoomDetail(Number(resolveCurrentTenantId(ctx)), ctx.params?.id) }; } catch (error) { return handleExamScheduleManagementError(ctx, error); }
  },
  async create(ctx: any) {
    const authUser = await requireAuthenticatedUser(ctx); if (!authUser?.id) return;
    try { ctx.body = { success: true, data: await createExamRoom(Number(resolveCurrentTenantId(ctx)), extractPayload(ctx), authUser) }; } catch (error) { return handleExamScheduleManagementError(ctx, error); }
  },
  async update(ctx: any) {
    const authUser = await requireAuthenticatedUser(ctx); if (!authUser?.id) return;
    try { ctx.body = { success: true, data: await updateExamRoom(Number(resolveCurrentTenantId(ctx)), ctx.params?.id, extractPayload(ctx), authUser) }; } catch (error) { return handleExamScheduleManagementError(ctx, error); }
  },
  async setActive(ctx: any) {
    const authUser = await requireAuthenticatedUser(ctx); if (!authUser?.id) return;
    try { ctx.body = { success: true, data: await setExamRoomActive(Number(resolveCurrentTenantId(ctx)), ctx.params?.id, extractPayload(ctx), authUser) }; } catch (error) { return handleExamScheduleManagementError(ctx, error); }
  },
};