...
import { getExamComponentDetail, listExamComponents } from '../services/exam-component';
...
export default factories.createCoreController(EXAM_COMPONENT_UID, () => ({
  async find(ctx: any) {
    const authUser = await requireAuthenticatedUser(ctx);
    if (!authUser?.id) return;

    const tenantId = resolveCurrentTenantId(ctx);
    const result = await listExamComponents(ctx.query || {}, tenantId);
    ctx.body = {
      success: true,
      data: result.rows,
  ...
  async findOne(ctx: any) {
    ...
    const tenantId = resolveCurrentTenantId(ctx);
    const entity = await getExamComponentDetail(ctx.params?.id, tenantId);
    if (!entity) return ctx.notFound('ExamComponent not found');
