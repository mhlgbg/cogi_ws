...
function pickAllowedPositionData(input: GenericRecord): GenericRecord {
	const allowedKeys = [
		'name',
		'code',
		'slug',
...
async function findPositionByParam(idParam: unknown) {
	...
	return strapi.db.query(POSITION_UID).findOne({
		where,
		populate: {
			tenant: {
...
async function assertPositionInCurrentTenant(ctx: any, tenantId: number | string) {
	const position = await findPositionByParam(ctx.params?.id);

	if (!position) {
		ctx.throw(404, 'Position not found');
	}

	const positionTenantRef = extractRelationRef(position?.tenant);
	if (toRelationKey(positionTenantRef) !== toRelationKey(tenantId)) {
		ctx.throw(403, 'You do not have permission to access this position');
	}

	return position;
}

export default factories.createCoreController(POSITION_UID, () => ({
	async find(ctx) {
		...
		const [rows, total] = await Promise.all([
			strapi.db.query(POSITION_UID).findMany({
				where,
			...
			strapi.db.query(POSITION_UID).count({ where }),
		]);
	...
	async findOne(ctx) {
		const tenantId = resolveCurrentTenantId(ctx);
		await assertPositionInCurrentTenant(ctx, tenantId);
	...
	async create(ctx) {
		...
		const payload = pickAllowedPositionData(data);
		payload.tenant = tenantId;

		const created = await strapi.db.query(POSITION_UID).create({
			data: payload,
		});

		const populatedCreated = await strapi.db.query(POSITION_UID).findOne({
			where: { id: created.id },
	...
	async update(ctx) {
		const tenantId = resolveCurrentTenantId(ctx);
		const position = await assertPositionInCurrentTenant(ctx, tenantId);

		const data = resolveRequestData(ctx);
		const payload = pickAllowedPositionData(data);
		payload.tenant = tenantId;

		const updated = await strapi.db.query(POSITION_UID).update({
			where: { id: position.id },
			data: payload,
		});

		const populatedUpdated = await strapi.db.query(POSITION_UID).findOne({
			where: { id: position.id },
			populate: normalizePopulateInput(ctx.query?.populate),
		});
