...
function resolveCategoryPopulate(ctx: any) {
...
export default factories.createCoreController(CATEGORY_UID, () => ({
	async find(ctx) {
		...
		const populate = resolveCategoryPopulate(ctx);
		...
		const [rows, total] = await Promise.all([
			...
			strapi.db.query(CATEGORY_UID).count({ where }),
		]);
	...
	async findOne(ctx) {
		const tenantId = resolveCurrentTenantId(ctx);
		const entity = await strapi.db.query(CATEGORY_UID).findOne({
			where: mergeTenantWhere(whereByParam(ctx.params?.id), tenantId),
			populate: resolveCategoryPopulate(ctx),
		});
	...
	async create(ctx) {
		...
		const created = await strapi.db.query(CATEGORY_UID).create({ data });
		const populatedCreated = await strapi.db.query(CATEGORY_UID).findOne({
			where: { id: created.id },
			populate: resolveCategoryPopulate(ctx),
		});
	...
	async update(ctx) {
		const tenantId = resolveCurrentTenantId(ctx);
		const existing = await strapi.db.query(CATEGORY_UID).findOne({
			where: mergeTenantWhere(whereByParam(ctx.params?.id), tenantId),
		});
		...
		const updated = await strapi.db.query(CATEGORY_UID).update({
			where: { id: existing.id },
			data,
		});
		const populatedUpdated = await strapi.db.query(CATEGORY_UID).findOne({
			where: { id: existing.id },
			populate: resolveCategoryPopulate(ctx),
		});
